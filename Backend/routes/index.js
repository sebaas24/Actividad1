...
const authenticateToken = (req, res, next) => {
  ...
  try {
    const tokenWhitoutBearer = token.replace("Bearer ", "");
    const verified = jwt.verify(tokenWhitoutBearer, process.env.TOKEN_SECRET);
    req.user = verified;
    next();
  } catch (err) {
...
router.put('/habits/:id', authenticateToken,async function(req, res) {
  try {
    const { title, description } = req.body;
